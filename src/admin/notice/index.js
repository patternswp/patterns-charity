/*CSS*/
/* global PatternsCharityLocalize */
...
class PatternsCharitySettingsApi {
	...
	bindEvents() {
		document.addEventListener( 'click', ( event ) => {
			...
			if ( target.closest( '.patterns-charity-gsn-close' ) ) {
				this.modifySettings( 'hide_get_started_notice', true );
			} else if (
				target.closest( '.patterns-charity-rn-permanent-close' )
			) {
				this.modifySettings( 'remove_review_notice_permanently', true );
			} else if (
				target.closest( '.patterns-charity-rn-temporary-close' )
			) {
				this.modifySettings(
				...
				const installBtn = target.closest(
					'.patterns-charity-install-plugins'
				);
				if ( installBtn ) {
					installBtn.classList.add( 'patterns-charity-processing' );
					this.installPlugins();
				}
			}
		} );
	...
	updateUI() {
		...
		if ( ! this.isLoadReview() ) {
			const reviewNoticeElement = document.getElementById(
				'patterns-charity-review-notice'
			);
