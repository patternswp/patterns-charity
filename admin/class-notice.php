...
class Patterns_Charity_Notice {
	...
	private function is_load_getting_started() {
		return ! patterns_charity_include()->get_settings( 'hide_get_started_notice' );
	}
	...
	private function is_load_review() {
		...
		$remove_review_notice_permanently         = patterns_charity_include()->get_user_meta( $user_id, 'remove_review_notice_permanently' );
		$remove_review_notice_temporary_date_time = patterns_charity_include()->get_user_meta( $user_id, 'remove_review_notice_temporary_date_time' );

		$theme_installed_date_time = patterns_charity_include()->get_settings( 'theme_installed_date_time' );
	...
	public function enqueue_resources() {
		...
		$unique_id = PATTERNS_CHARITY_THEME_NAME . '-notice';
		...
		$deps_file = PATTERNS_CHARITY_PATH . 'build/admin/notice/notice.asset.php';

		/*Fallback dependency array*/
		$dependency = array();
		$version    = PATTERNS_CHARITY_VERSION;
		...
		wp_enqueue_script( $unique_id, PATTERNS_CHARITY_URL . 'build/admin/notice/notice.js', $dependency, $version, true );
		wp_enqueue_style( $unique_id, PATTERNS_CHARITY_URL . 'build/admin/notice/notice.css', array(), $version );
		wp_style_add_data( $unique_id, 'rtl', 'replace' );

		/* Localize */
		$localize = apply_filters(
			'patterns_charity_notice_localize',
			array(
				'version'             => $version,
				'nonce'               => wp_create_nonce( 'wp_rest' ),
				'rest_url'            => get_rest_url(),
				'recommended_plugins' => patterns_charity_get_recommended_plugins(),
				'theme_info_url'      => esc_url( menu_page_url( PATTERNS_CHARITY_THEME_NAME, false ) ),
			)
		);

		wp_set_script_translations( $unique_id, PATTERNS_CHARITY_THEME_NAME );
		wp_localize_script( $unique_id, 'PatternsCharityLocalize', $localize );
	}
	...
	public function add_theme_data() {
		...
		if ( ! $theme_installed_date_time ) {
			patterns_charity_update_options( 'theme_installed_date_time', time() );
		}
	}
	...
	public function remove_theme_data() {
		...
		delete_option( PATTERNS_CHARITY_OPTION_NAME );

		// Delete the user meta for all users.
		delete_metadata(
			'user',
			0,
			PATTERNS_CHARITY_OPTION_NAME,
			'',
			true
		);
	}
...
/**
 * Return instance of  Patterns_Charity_Notice class
 *
 * @since 1.0.0
 *
 * @return Patterns_Charity_Notice
 */
function patterns_charity_notice() { //phpcs:ignore
	return Patterns_Charity_Notice::instance();
}
patterns_charity_notice()->run();
